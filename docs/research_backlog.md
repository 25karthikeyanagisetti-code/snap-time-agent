...
- `tag_aware_injection` — Extend the tag-aware approach to `inject_recalled_emotion`: when the highest-impact recalled memory is guilt-tagged but its stored guilt has been laundered, inject a tag-keyed floor template (or max(stored, floor)) instead of the literal decayed channels. Tests whether the residual β_guilt=0.50 ep0 collapse (58% under tag-aware recall vs 78% symmetric baseline) is closed by also tag-keying the injection pathway. (Elevated 2026-05-09 from the tag_aware_recall residual.)
